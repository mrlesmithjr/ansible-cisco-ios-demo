...
def is_switchport(name, module):
    ...
    if intf_type in ('ethernet', 'portchannel'):
        config = run_commands(
            module, ['show interface {0} switchport'.format(name)])[0]
...
def get_switchport(name, module):
    config = run_commands(
        module, ['show interface {0} switchport'.format(name)])[0]
    mode = re.search(r'Administrative Mode: (?:.* )?(\w+)$',
                     config, re.M).group(1)
...
def get_switchport_config_commands(name, existing, proposed, module):
    ...
    if proposed_mode == 'access':
        ...
        tv_check = existing.get(
            'trunk_vlans_list') == proposed.get('trunk_vlans_list')
